...
bool ofxOpenNI2Grabber::setup(ofxOpenNI2GrabberSettings settings_, bool bUseONIThread)
{
	settings = settings_;
	...
	if (settings.doDepth) 
	{
		...
		if (depthSource.setup(deviceController, bUseONIThread))
		{
	...
	if (settings.doColor) 
	{
		if (rgbSource.setup(deviceController, bUseONIThread)) 
		{
