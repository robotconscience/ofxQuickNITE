...
void DeviceController::setup(ofxOpenNI2GrabberSettings _settings)
{
	...
	{
		const char* deviceUri = (_settings.deviceURI == "" ? openni::ANY_DEVICE : _settings.deviceURI.c_str());
...
//--------------------------------------------------------------
vector< DeviceInfo > DeviceController::enumerateDevices(){
    //ofxOpenNI::initialize();
    
    static vector< DeviceInfo > devices;
    devices.clear();
    static Array< DeviceInfo > oniDeviceArray;
    
    OpenNI::enumerateDevices( &oniDeviceArray );
    for (int i=0; i<oniDeviceArray.getSize(); i++){
        devices.push_back( oniDeviceArray[i] );
        ofLogVerbose() << "Device URI: "			<< oniDeviceArray[i].getUri();
        ofLogVerbose() << "Device Vendor: "			<< oniDeviceArray[i].getVendor();
        ofLogVerbose() << "Device Name: "			<< oniDeviceArray[i].getName();
        ofLogVerbose() << "Device USB Vendor ID "	<< oniDeviceArray[i].getUsbVendorId();
        ofLogVerbose() << "Device USB Product ID: "	<< oniDeviceArray[i].getUsbProductId();
    }
    return devices;
}