	...
	for linked_name in ${lines} ; do 
		for local_lib in ${libs[@]}; do
			...
			if [ "$in" != "" ] ; then
				...
				if [ "$check_id" != "" ] ; then
				 	install_name_tool -id  "@executable_path/./OpenNI2/Drivers/${local_lib}" ${lib}
					echo install_name_tool -id  "@executable_path/./OpenNI2/Drivers/${local_lib}" ${lib}
				else 
					install_name_tool -change "${linked_name}" "@executable_path/./OpenNI2/Drivers/${local_lib}" ${lib}
					echo  install_name_tool -change "${linked_name}" "@executable_path/./OpenNI2/Drivers/${local_lib}" ${lib}
				fi 
			fi	
		done
	done
